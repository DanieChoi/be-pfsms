...
public class AuthServiceImpl implements AuthService {
  ...
  public ResponseEntity<? super PutSignInResponseDto> signIn(
    PutSignInRequestDto requestBody,
    ...
    try {
      ...
      UserEntity rsUser = userRepository.findByLoginId(strLoginId);
      if (rsUser == null) return PutSignInResponseDto.notExistLoginId();
      ...
      if(!isFirstLogin) {
        ...
        if(elapsedDay >= rsUser.getPasswordChangeCycleDayCount()) return PutSignInResponseDto.exceedingElapsedTime();
      ...
      if(!isMatched) {
        ...
        if(rsUser.getPwdErrNotm() >= 3) {
          return PutSignInResponseDto.passwordError3TimesOrMore();
        }
        return PutSignInResponseDto.passwordError();
      }
    ...
    return PutSignInResponseDto.success(token);
  }
